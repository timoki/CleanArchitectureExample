 ...
 * 이 클래스의 기능 : 클래스명 그대로 이해하면 된다 Local(Room) 에 ConfigData 를 Insert 함
 * */
class InsertConfigLocalUseCase @Inject constructor(
    private val configRepository: ConfigRepository
) {
    suspend operator fun invoke(configData: ConfigDataModel) {
        return configRepository.insertConfigDataLocal(configData)
    }
}